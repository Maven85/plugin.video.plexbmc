...
def addGUIItem(url, details, extraData, context=None, folder=True):
    ...
    liz.setArt({'thumb': extraData.get('thumb', GENERIC_THUMBNAIL)})
...
def playPlaylist (server, data):
    ...
    for track in TrackTags:
        ...
        liz.setArt({'icon': data['full_data'].get('thumbnailImage', ''), 'thumb': data['full_data'].get('thumbnailImage', '')})
...
def playLibraryMedia(vids, override=False, force=None, full_data=False, shelf=False):
    ...
    if streams['full_data']:
        ...
        item.setArt({'icon': streams['full_data'].get('thumbnailImage', ''), 'thumb': streams['full_data'].get('thumbnailImage', '')})
